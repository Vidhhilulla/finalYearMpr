...
export class CommentsComponent implements OnInit {
  ...
  badWordsSet: Set<string> = new Set([
    'badword1',
    'badword2',
    'illegalword',
    // Add more here...
  ...
  containsBadWords(text: string): boolean {
    ...
    return words.some(word => this.badWordsSet.has(word));
  }
  ...
  addComment() {
    if (this.commentForm.valid) {
      ...
      if (this.containsBadWords(comment)) {
        alert('Your comment contains inappropriate content and cannot be posted. you need to take permission from the user to post this.');
        return;
      }
      let commentObj = {
